...
namespace NSE.WebApp.MVC.Controllers
{
    public class IdentidadeController : BaseController
    {
        ...
        public async Task<IActionResult> Login(UsuarioLogin usuarioLogin, string returnUrl = null)
        {
            ...
            if (ResponsePossuiErros(usuarioRespostaLogin.ResponseResult))
        ...
        private async Task RealizarLogin(UsuarioRespostaLogin login)
        {
            var token = ObterTokenFormatado(login.AccessToken);

            var claims = new List<Claim>();
            claims.Add(new Claim("JWT", login.AccessToken));
